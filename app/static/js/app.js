...
async function registerServiceWorker() {
  await navigator.serviceWorker.register("/cdn/static/js/sw.js");
  updateUI();
}
...
async function subscribeToPush() {
  ...
  postToServer("/push/add-subscription", subscription);
  updateUI();
}
...
async function unsubscribeFromPush() {
  ...
  postToServer("/push/remove-subscription", {
...
async function notifyMe() {
  const registration = await navigator.serviceWorker.getRegistration();
  const subscription = await registration.pushManager.getSubscription();
  postToServer("/push/notify-me", { endpoint: subscription.endpoint });
}
